...
- The **Avalonia GUI** is the second front-end (cross-platform incl. mobile), built on `Nfty.Core`; design mockups live in `docs/design/mockups/` — `explorer.html` (the primary screen), `landing.html` (the pre-open default view), `help.html` (the quick-reference view), the three `wizard-*.html`, and `ingredient-editor.html`. Their style is **locked** and they are the 1:1 reference: the token block is shared verbatim across all of them, so a new hex literal in any of them means the design has drifted. **Never edit a mockup to match the app** — the app moves. Keep `Nfty.Core` free of UI/CLI dependencies so both front-ends can share it.
...
- **Token brushes only in Views.** No raw hex outside `Themes/Tokens.axaml`; a new colour goes in **both** dictionaries. Avalonia hex is `#AARRGGBB` — the mockup CSS is `#RRGGBBAA`, so alpha moves to the front.
