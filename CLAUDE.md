...
- The **Avalonia GUI** is the next sub-project (cross-platform incl. mobile), built on `Nfty.Core`; design mockups live in `docs/design/mockups/` — `explorer.html` (the primary screen), `landing.html` (the pre-open default view), and `help.html` (the quick-reference view, reachable from landing), each with a spec in `docs/superpowers/specs/`. Their style is **locked**: the token block is shared verbatim across all three, so a new hex literal in any of them means the design has drifted. Keep `Nfty.Core` free of UI/CLI dependencies so both front-ends can share it.
