...
/// <remarks>
/// A variant carries a grayscale <see cref="Map"/> always, and a full-colour <see cref="Color"/> once
/// it has been painted or imported in colour mode. Which one is authoritative is decided by the
...
/// non-destructive save needs. Painting a Dynamic layer in colour leaves its value-map untouched —
/// so the original ingredient stays byte-identical on disk — while the colour map becomes a new
...
public sealed class VariantDraft
{
    ...
    /// <summary>The editable full-colour pixels, or null while this variant has only ever been a
    /// value-map. Set when colour mode is entered, and the only thing a Custom layer exports.</summary>
    ...
    /// <param name="color">Its full-colour pixels, or null for a variant not authored in colour.</param>
    ...
    /// <summary>The variant's colour pixels, widening its value-map on first use so entering colour
    /// mode shows the drawing that is already there rather than a blank canvas. Idempotent: once a
    /// colour map exists it is returned untouched, so re-entering colour mode never discards paint.</summary>
    /// <returns>This variant's colour map.</returns>
