...
public static class VariantPreview
{
    ...
    /// <param name="modelOverride">Renders in a different color model than the ingredient declares,
        ...
        string? colorSpec = null, ColorModel? modelOverride = null)
    {
        ...
        if (colorSpec is null)
            throw new InvalidOperationException(
                $"A color is required to preview '{ingredient.Manifest.Name}': it is a "
    ...
    /// between. The spec overload <i>cannot</i> match that for a rolled color — every color spec
    /// resolves back through 8-bit RGB, so a hue that fell between two representable colors comes
    ...
    /// being shown.</para>
    ///
    /// <para>A Static layer needs no such overload: its color is an author-written spec, so
    /// generation resolves the same string through the same parser and the spec overload is already
    /// exact. Only a <i>rolled</i> color has a value with no exact spelling.</para>
    /// </summary>
    /// <param name="ingredient">The layer the variant belongs to; supplies the kind and the
    /// color model.</param>
    /// <param name="variantId">Which variant of it to render.</param>
    ...
    /// <returns>The rendered image; the caller owns it. A Custom layer is cloned as-is and the color
    ...
    /// <summary>Resolves the variant's image and settles the one case that needs no color at all.
    /// Custom layers are full-color RGBA composited as-is and are never colorized — their
    /// Colorization is always null — so a preview claiming to show what generation renders must pass
    /// them through rather than applying a color generation never applies.</summary>
    ...
    /// <summary>The color model to render in: the caller's override, else the layer's own.
    ///
    ...
    /// where the color ranges, the entry weights and the DNA quantize steps live, so an override
    ...
    private static ColorModel ModelFor(LoadedIngredient ingredient, ColorModel? modelOverride)
    {
        var colorization = ingredient.Manifest.Colorization
            ?? throw new InvalidOperationException(
                ...
                + "colorization block, so there is no color model to render it in.");
        return modelOverride ?? colorization.Model;
    }

    /// <summary>Whether <paramref name="ingredient"/> needs a color to be previewed. Custom does
