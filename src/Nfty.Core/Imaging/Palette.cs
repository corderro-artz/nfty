...
/// different set of colours to pick from and changes no pixel that has already been painted —
/// recolouring existing art through a ramp is a separate feature with its own mapping curve, preview
/// and undo entry, and is deliberately not this.</summary>
public enum PaletteMode
{
    /// <summary>Ten ascending greys — what a value-map is authored in, since a Dynamic or Static
    /// layer takes its lightness from the grey and its hue and saturation from the colorization.</summary>
    Grayscale,

    /// <summary>Ten hues around the wheel — full-colour painting, which only ever saves as a
...
/// CLI and the GUI must agree on what "the fifth grey" is, and two copies is how that stops being
/// true. The ramps are also <em>computed</em> rather than typed out, so the greys are reproducible
...
/// <param name="Swatches">The user's saved colours, in the order they were saved. Unaffected by
/// <see cref="Mode"/>: a swatch mixed in colour mode is still there in grayscale mode, because a
/// saved colour is user data rather than a property of the ramp.</param>
public sealed record Palette(PaletteMode Mode, IReadOnlyList<RgbColor> Swatches)
{
    ...
    /// <summary>Ten ascending greys from black to white, evenly spaced across the full 0..255 range.
    ///
    /// <para>Both endpoints are included — unlike the half-open ranges <c>ColorRoller</c> samples,
    /// this is a fixed ten-entry table rather than a sampled interval, and a grey ramp that could
    /// not reach white would be missing the colour authors reach for most.</para></summary>
    ...
    /// <returns>Exactly <see cref="Slots"/> colours.</returns>
    ...
    /// <summary>Saves a swatch, appending it. Re-saving a colour already present is a no-op rather
    ...
    /// <summary>
    /// Reads swatches out of the prefixed colour specs a manifest or a store file holds.
    ///
    ...
    /// <returns>The colours that parsed, in order.</returns>
    ...
    /// <returns>One spec per colour, in order.</returns>
