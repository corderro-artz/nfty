...
public class ExplorerViewModelTests
{
    ...
    internal static Func<LoadedIngredient, LoadedCookBook, string, IngredientEditorViewModel> LooseEditorFactory(
        ...
        (ing, book, path) => new IngredientEditorViewModel(ing, book.Recipes[0], book, new ImageBridge(),
            nav, session, dialogs, new FilePickerService(), looseSavePath: path);
    ...
    private static ExplorerViewModel Make()
    {
        var nav = new FakeNav();
        var dialogs = new FakeDialogs();
        var session = new CookBookSession();
        return new ExplorerViewModel(TwoRecipeBook(), nav, dialogs, new ImageBridge(), EditorFactory(nav), CookFactory(dialogs), session,
    ...
    public void Tree_is_built_from_the_cookbook_recipes_and_ingredients()
    {
        ...
        var dialogs = new FakeDialogs();
        var session = new CookBookSession();
        using var vm = new ExplorerViewModel(TwoRecipeBook(), nav, dialogs, new ImageBridge(), EditorFactory(nav), CookFactory(dialogs), session,
            new FilePickerService(), LooseEditorFactory(nav, session, dialogs), new StatusService());
    ...
    {
        using var vm = Make();              // in-memory book (Make's session has no source path)
    ...
    public async Task Editor_save_rebuilds_the_tree_and_reselects_the_ingredient()
    {
        ...
        try
        {
            ...
            using var explorer = new ExplorerViewModel(session.Current!, nav, dialogs,
