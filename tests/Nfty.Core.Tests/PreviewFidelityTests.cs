...
///
/// <para>It was not true for a rolled color. A <see cref="PreviewLayer"/> named its color as a spec
/// string, and every spec resolves through 8-bit RGB, so a hue that fell between two representable
/// colors came back as the nearest one and the preview landed a rounding step off the asset. About a
...
public class PreviewFidelityTests
{
    ...
    /// <summary>A value-map layer whose color is rolled from a wide range, so the rolled hue is a
    ...
    // Draws are scripted rather than seeded (see ScriptedRng) so the rolled color is stated by the
    // test rather than discovered. The budget matters here specifically: a color roll costs THREE
    ...
    /// the rolled color ignored and the spec rendered instead, this theory fails at
    /// <c>0.123456789</c> and passes at the other three, because most rolled colors happen to
    ...
    public void A_rolled_layer_previews_as_the_exact_pixels_generation_would_draw(double draw)
    {
        ...
        // ForIngredient spends first; the three after it are the same color draws as above, so any
        // difference in the pixels is the color's TRIP through PreviewLayer and nothing else.
    ...
    /// beside the rolled color does <b>not</b> resolve back to it. Asserted on the values rather than
    /// on pixels — a hue drift of a quarter-degree may or may not move an 8-bit channel for any
    /// particular gray, so a pixel-level version of this could pass by luck on a kinder test image
    /// while the defect was still there.
    /// </summary>
    [Fact]
    public void The_printed_spec_cannot_spell_the_rolled_color_exactly()
    {
        using var ing = Dynamic("body");
    ...
    /// <summary>
    /// The field earns its place: rendering the same layer with the rolled color and without it
    ...
    /// </summary>
    [Fact]
    public void Dropping_the_rolled_color_changes_what_is_drawn()
    {
        using var ing = Dynamic("body");
    ...
    /// <summary>
    /// A Static layer needs none of this and must not acquire it. Its color is a spec the author
    ...
    /// </summary>
    [Fact]
    public void A_static_layer_carries_no_rolled_color_because_its_spec_is_already_exact()
    {
        ...
        // Exactly one draw is offered — the variant's. A color draw would overrun the script and
        // throw, so "Static consumes no RNG for its color" is enforced by the budget, not just
    ...
    /// <summary>
    /// A Custom layer is composited as-is and is never colorized, so neither color field means
    /// anything for it — and a rolled color arriving on one must not change a single pixel. This is
    /// the kind-boundary the rest of the engine already holds; it holds here too.
    /// </summary>
    [Fact]
    public void A_custom_layer_ignores_a_rolled_color_entirely()
    {
        using var ing = new LoadedIngredient
        {
